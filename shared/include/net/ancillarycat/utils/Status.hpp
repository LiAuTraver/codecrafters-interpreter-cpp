...
public:
  ...
  StatusOr(StatusOr &&that) noexcept
      : base_type(std::move(that).as_status()), my_value(std::move(that.my_value)) {}
...
inline AC_CONSTEXPR20 Status FileNotFoundError(
    const std::string_view message,
    const std::source_location &location = std::source_location::current()) {
...
inline AC_CONSTEXPR20 Status InvalidArgument(
    const string_view message,
    const std::source_location &location = std::source_location::current()) {
  return {Status::kInvalidArgument, message, location};
}

inline AC_CONSTEXPR20 Status CommandNotFound(
    const string_view message,
    const std::source_location &location = std::source_location::current()) {
...
inline AC_CONSTEXPR20 Status ParseError(
    const string_view message,
    const std::source_location &location = std::source_location::current()) {
  return {Status::kParseError, message, location};
}

inline AC_CONSTEXPR20 Status NotImplementedError(
    const string_view message,
    const std::source_location &location = std::source_location::current()) {
